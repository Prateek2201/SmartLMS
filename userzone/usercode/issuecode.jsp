<%-- 
    Document   : issuecode
    Created on : Mar 18, 2019, 12:51:16 AM
...
<%
    ...
    if(book!=null)
    {
        ...
        {
            out.print("<script>alert('Book ID not found');window.location.href='../issue.jsp';</script>");
        }
    }
    else if(student!=null) 
    {
        ...
        {
        String query="select sid,sname,fname,course,branch,year,semester from student where sid='"+sid+"'";
        ConnectDb ob=new ConnectDb();
        ResultSet rs=ob.selectQuery(query);
        if(rs.next()==true)
        {
            ...
            session.setAttribute("course", rs.getString(4));
            session.setAttribute("branch", rs.getString(5));
            session.setAttribute("year", rs.getString(6));
            session.setAttribute("semester", rs.getString(7));            
            response.sendRedirect("../issue.jsp");
        }
        else
        {
            out.print("<script>alert('Student ID not found');window.location.href='../issue.jsp';</script>");
        }
        }
        else
        {
            out.print("<script>alert('Invalid Student_ID');window.location.href='../issue.jsp';</script>");
        }
    }
    else if(issue!=null)
    {
        ...
        if(ob.executeNonQuery(query)==true)
        {
            ...
            out.print("<script>alert('Book Issued successfully');window.location.href='../issue.jsp';</script>");
            }
            else{
        ...
        {
//            out.print("<script>alert('Book with Book_ID="+bid+" is not available');window.location.href='../issue.jsp';</script>");
            out.print("<script>alert('Already issued one copy of this book!');window.location.href='../issue.jsp';</script>");
        }
    }
    else
    {
